...
export default function TournamentsPage() {
  ...
  const fetchTournaments = async () => {
    ...
    try {
      ...
      } else if (data.started && Array.isArray(data.started)) {
        tournamentsData = data.started;
      } else if (data.created && Array.isArray(data.created)) {
        tournamentsData = data.created;
      } else if (data.finished && Array.isArray(data.finished)) {
        tournamentsData = data.finished;
        ...
        <div className="space-y-4">
          {!tournaments || tournaments.length === 0 ? (
            ...
            tournaments.map((tournament) => {
              // Debug log to see tournament structure
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    ...
                    <p className="text-lg font-semibold text-gray-900 capitalize">
                      {typeof tournament.variant === 'string' ? tournament.variant : tournament.variant?.key || 'Unknown'}
                    </p>
                  </div>
                </div>
